...
const generateFileName = (date) => {
  return `tasksHistory_${date.getDate().toString().padStart(2, "0")}_${(
    date.getMonth() + 1
  )
    .toString()
    .padStart(2, "0")}_${date.getFullYear().toString().slice(-2)}.json`;
};
...
const writeTasksHistory = async function (tasksHistory, overwrite = false) {
  ...
  const date =
    tasksHistory.length > 0 && tasksHistory[0]?.completedAt
      ? new Date(tasksHistory[0].completedAt)
      : new Date();
...
const pushCompletedTask = async function (tasks) {
  ...
  const date =
    tasksArray.length > 0 && tasksArray[0]?.completedAt
      ? new Date(tasksArray[0].completedAt)
      : new Date();
  ...
  try {
    ...
    if (tasksArray.length > 0)
      tasksArray.forEach(
        (task) => (task.currentProfile = this.state.currentProfile.userName)
      );
...
const readProfilesData = async () =>
  JSON.parse(await fs.readFile("./data/allProfilesData.json"));
...
const getProfileByUserName = async function (userName) {
  ...
  const profile = allProfilesData.find(
    (profile) => profile.userName === userName
  );
  if (!profile)
    throw new Error(
      `There is no profile in allProfilesData.json by userName : ${userName}.`
    );
  return profile;
};
const getUserDataPathByUserName = async function (userName) {
  ...
  if (!userDataPath)
    throw new Error(
      `userDataPath property does not exists on ${userName}'s profile in allProfilesData.json.`
    );

  // Check if file exists
  if (!(await fs.pathExists(userDataPath)))
    throw new Error(
      `User data file does not exist for user: ${userName}, at path userDataPath: ${userDataPath}`
    );
...
const addNewProfile = async function () {
  ...
  userInput = await this.utils.askUser(
    "Enter type of profile: 1 for 'agent', 2 for 'scrper' or 3 for 'resource': "
  );
  if (userInput === "1") newProfile.type = "agent";
    ...
    if (!result)
      throw new Error(
        `Failed to add new resource profile: ${JSON.stringify(newProfile)}`
      );
    return false;
  ...
  if (newProfile.type === "agent")
    newProfile.linkedResourceUserName = "notYetDefined";
...
const removeDueTask = async function (userName, dueTaskObj) {
  console.log(
    `Removing due task for user: ${userName}, ${JSON.stringify(dueTaskObj)}`
  );
  ...
  for (const profile of profilesData) {
    if (profile.userName === userName && profile.type !== "resource") {
      ...
      const index = profile.dueTasks.findIndex(
        (task) =>
          task.parentModuleName === dueTaskObj.parentModuleName &&
          task.actionName === dueTaskObj.actionName &&
          task.argumentsString === dueTaskObj.argumentsString
      );
      ...
      if (dueTaskIndex === -1) {
        throw new Error(
          `This particular due task ${JSON.stringify(
            dueTaskObj
          )} for user: ${userName} doesn't exists in file ${
...
const removeDuplicatesFormAutomatedFollowArray = (arr) =>
  utils.removeDuplicates(arr, ["userName"], "date");

const updateDatabaseOnFollow = async function (userObject) {
  console.log(
    `Adding ${JSON.stringify(userObject)} to ${
      this.state.currentProfile.userName
    }.automatedFollow: [].`
  );

  const currentUserData = await readUserProfileData.call(
    this,
    this.state.currentProfile.userName
  );
  ...
  currentUserData.automatedFollow = removeDuplicatesFormAutomatedFollowArray(
    currentUserData.automatedFollow
  );
  ...
  this.state.currentProfile = await readUserProfileData.call(
    this,
    this.state.currentProfile.userName
  );
  ...
  if (!currentfollowDueTask)
    throw new Error(
      `currentfollowDueTask {"argumentsString"===${
        userObject.userName
      }, "actionName" === "follow"} not found in ${JSON.stringify(
...
const readTargetStringsToScrape = async () =>
  JSON.parse(
    await fs.readFile("./data/instaScrapedData/targetStringsToScrape.json")
  );
...
const writeTargetStringsToScrape = async function (targetStringsToScrape) {
  if (!Array.isArray(targetStringsToScrape))
    throw new Error("targetStringsToScrape must be an array");
  await fs.writeFile(
    "./data/instaScrapedData/targetStringsToScrape.json",
    JSON.stringify(targetStringsToScrape, null, 2)
  );
  return true;
};

const readTargetStringsAlreadyScraped = async () =>
  JSON.parse(
    await fs.readFile(
      "./data/instaScrapedData/targetStringsAlreadyScraped.json"
    )
  );
readTargetStringsAlreadyScraped.shouldStoreState =
  "targetStringsAlreadyScraped";

const writeTargetStringsAlreadyScraped = async function (
  targetStringsAlreadyScraped
) {
  if (!Array.isArray(targetStringsAlreadyScraped))
    throw new Error("targetStringsAlreadyScraped must be an array");
  await fs.writeFile(
    "./data/instaScrapedData/targetStringsAlreadyScraped.json",
    JSON.stringify(targetStringsAlreadyScraped, null, 2)
  );
  return true;
};
...
const writeScrapedTarget = async function () {
  ...
  await fs.writeFile(
    this.state.targetToScrape.dataPath,
    JSON.stringify(this.state.targetToScrape, null, 2)
  );
  return true;
};
...
const addNewTargetString = async function () {
  let userInput = await this.utils.askUser(
    `Do you want to add new Target to scrape? , 1  or y for 'yes' : `
  );
  if (userInput.toLowerCase().trim() !== "1" || userInput !== "y") return false;

  this.state.targetStringsToScrape ||= await readTargetStringsToScrape();
  this.state.targetStringsAlreadyScraped ||=
    await readTargetStringsAlreadyScraped();

  // Create a new target object
  let newTarget = {};

  newTarget.targetString = await this.utils
    .askUser(`Enter target string: `)
    .toLowerCase()
    .trim();
  if (
    this.state.targetStringsAlreadyScraped.includes(
      (targetAlreadyScraped) =>
        targetAlreadyScraped.targetString === newTarget.targetString
    )
  ) {
    console.log(
      `Profile with user name: ${newTarget.targetString} already exists`
    );
    return false;
  }
  ...
  userInput = undefined;
  userInput = await this.utils.askUser(
    "Enter targetType of Target: 1 for 'teraBox', 2 for 'movies' or 3 for 'other': "
  );
  ...
  userInput = undefined;
  userInput = await this.utils.askUser(
    "Target is a Profile, 1  or y for 'yes' : "
  );
  if (userInput.toLowerCase().trim() === "1" || userInput === "y")
    newTarget.type = "Profile";
  else newTarget.type = "Post";

  newTarget.dataPath = newTarget.profileTarget = await this.utils.askUser(
    "Enter profile target: "
  );
  if (
    this.state.profilesData.find(
      (profile) => profile.profileTarget == newTarget.profileTarget
    )
  )
    throw new Error(
      `Profile with target ${newTarget.profileTarget} already exists`
    );
...
const updateScrapedData = async function () {
  ...
  if (this.state.targetToScrape.type === "Profile") {
    ...
    const indexOfTargetToScrape = this.state.targetStringsToScrape.findIndex(
      (targetToScrape) =>
        targetToScrape.targetString === this.state.targetToScrape.targetString
    );
    if (indexOfTargetToScrape !== -1)
      this.state.targetStringsToScrape.splice(indexOfTargetToScrape, 1);
    ...
    await writeTargetStringsToScrape.call(
      this,
      this.state.targetStringsToScrape
    );
    await writeTargetStringsAlreadyScraped.call(
      this,
      this.state.targetStringsAlreadyScraped
    );
...
const filterProfilesToAutomate = async function () {
  ...
  this.state.profilesToLoop = this.state.profilesData.filter(
    (profile) => profile.type === "agent" && profile.dueTasks.length !== 0
  );
  this.state.profilesToLoop.sort(
    (a, b) => b.dueTasks.length - a.dueTasks.length
  );
...
module.exports = {
  ...
  writeTargetStringsAlreadyScraped: catchAsync(
    writeTargetStringsAlreadyScraped
  ),
