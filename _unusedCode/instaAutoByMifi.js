...
const Instauto = async (db, browser, options) => {
  ...
  async function followUserRespectingRestrictions({ username, skipPrivate = false }) {
    ...
    if (
      ...
      !shouldFollowUser({ username, isVerified, isBusinessAccount, isProfessionalAccount, fullName, biography, profilePicUrlHd, externalUrl, businessCategoryName, categoryName }) === true
    ) {
  ...
  async function setLang(short, long, assumeLoggedIn = false) {
    ...
    try {
      ...
      logger.error("Failed to set language, By JN not trying fallback (cookie)", err);
  ...
  */
  ...
  console.log(`Just before Error i.e. page.$x is not a function the page is as :`);
  console.log(page);
  console.log(`--------------------------------------`);

  await tryPressButton(await page.$$('xpath///button[contains(text(), "Accept")]'), "Accept cookies dialog");
  await tryPressButton(await page.$$('xpath///button[contains(text(), "Only allow essential cookies")]'), "Accept cookies dialog 2 button 1", 10000);
  await tryPressButton(await page.$$('xpath///button[contains(text(), "Allow essential and optional cookies")]'), "Accept cookies dialog 2 button 2", 10000);
  ...
  if (!(await isLoggedIn())) {
    ...
    await tryPressButton(await page.$$('xpath///button[contains(text(), "Log In")]'), "Login form button");
    ...
    // Mobile version https://github.com/mifi/SimpleInstaBot/issues/7
    await tryPressButton(await page.$$('xpath///button[contains(text(), "Save Info")]'), "Login info dialog: Save Info");
  ...
  */
  ...
  await tryPressButton(await page.$$('xpath///button[contains(text(), "Not Now")]'), "Turn on Notifications dialog");
  ...
  async function unfollowOldFollowed({ ageInDays, limit } = {}) {
    ...
    function condition(username) {
      return getPrevFollowedUser(username) && !excludeUsers.includes(username) && (new Date().getTime() - getPrevFollowedUser(username).time) / (1000 * 60 * 60 * 24) > ageInDays;
    }
